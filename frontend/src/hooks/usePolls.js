...
import { getPolls, createPoll, updatePoll, votePoll, deletePoll, saveInsight, addComment, deleteComment } from '../api/polls'
...
export function usePolls() {
      ...
      try {
        ...
        if (msg.type === 'POLL_UPDATED' && msg.poll) {
          setPolls(prev => {
            ...
            if (exists) return prev.map(p => p._id === msg.poll._id ? msg.poll : p)
  ...
  const vote = async (pollId, optionId) => {
    setPolls(prev => prev.map(p => p._id !== pollId ? p : {
      ...p,
      options: p.options.map(o => o._id === optionId ? { ...o, votes: o.votes + 1 } : o),
  ...
  const postComment = async (pollId, text, author) => {
    ...
    setPolls(prev => prev.map(p => p._id !== pollId ? p : { ...p, comments: [...(p.comments||[]), comment] }))
    return comment
  }

  const removeComment = async (pollId, commentId) => {
    await deleteComment(pollId, commentId)
    setPolls(prev => prev.map(p => p._id !== pollId ? p : { ...p, comments: (p.comments||[]).filter(c => c._id !== commentId) }))
  }

  return { polls, loading, error, filters, setFilters, addPoll, editPoll, vote, removePoll, updateInsight, postComment, removeComment, reload: load }
}
